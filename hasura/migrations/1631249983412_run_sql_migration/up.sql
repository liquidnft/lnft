...
BEGIN
    IF NEW.type = 'bid' THEN
        SELECT transactions.amount, artworks.bid_increment, artworks.auction_end, artworks.reserve_price
        FROM transactions
        ...
        LIMIT 1
        INTO bid, bid_increment, auction_end, reserve_price;
        
      IF NEW.amount < reserve_price OR (bid is not null and NEW.amount < (bid + bid_increment) AND auction_end >= NOW()) THEN
          RAISE EXCEPTION 'Bid of % below minimum bid of %', NEW.amount, GREATEST(reserve_price, bid + bid_increment);
      END IF;
    END IF;
