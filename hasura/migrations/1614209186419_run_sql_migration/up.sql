CREATE OR REPLACE VIEW "public"."collectors" AS 
 SELECT u.id,
    ...
    ( SELECT count(*) AS count
           FROM artworks a
          WHERE (u.id = a.owner_id) and u.id != a.artist_id) AS owned,
    ( SELECT count(*) AS count
           FROM transactions t
          WHERE (((u.id = t.user_id) AND (t.type = 'purchase'::text)))) AS collected,
    ( SELECT count(*) AS count
           FROM (transactions t
    ...
    ( SELECT abs(sum(((t.amount)::numeric / (100000000)::numeric))) AS sum
           FROM transactions t
   ...
   WHERE ( SELECT count(*) AS count
           FROM transactions t
          WHERE (((u.id = t.user_id) AND (t.type = 'purchase'::text))))  > 0
