...
const secp256k1$2 = new EC$1('secp256k1');
...
const n = secp256k1$2.curve.n;
const nDiv2 = n.shrn(1);
const G = secp256k1$2.curve.g;
...
function decodeFrom (P) { return secp256k1$2.curve.decodePoint(P) }
...
exports.Network = typeforce_1.compile({
  ...
  wif: typeforce_1.UInt8,
  assetHash: typeforce_1.String,
  confidentialPrefix: typeforce_1.UInt8,
});
...
exports.ConfidentialCommitment = typeforce_1.BufferN(33);
exports.ConfidentialValue = typeforce_1.BufferN(9);
exports.BufferOne = typeforce_1.BufferN(1);
...
exports.Object = typeforce_1.Object;
...
function encodingLength (i) {
  return i < require$$0$1.OP_PUSHDATA1 ? 1
  : i <= 0xff ? 2
  : i <= 0xffff ? 3
  : 5
}
...
function encode (buffer, number, offset) {
  ...
  // 8 bit
    ...
    buffer.writeUInt8(require$$0$1.OP_PUSHDATA1, offset);
    buffer.writeUInt8(number, offset + 1);

  // 16 bit
  } else if (size === 3) {
    buffer.writeUInt8(require$$0$1.OP_PUSHDATA2, offset);
    buffer.writeUInt16LE(number, offset + 1);

  // 32 bit
  } else {
    buffer.writeUInt8(require$$0$1.OP_PUSHDATA4, offset);
...
function decode (buffer, offset) {
  ...
  // ~6 bit
  if (opcode < require$$0$1.OP_PUSHDATA1) {
    number = opcode;
    size = 1;

  // 8 bit
  } else if (opcode === require$$0$1.OP_PUSHDATA1) {
    if (offset + 2 > buffer.length) return null
    number = buffer.readUInt8(offset + 1);
    size = 2;

  // 16 bit
  } else if (opcode === require$$0$1.OP_PUSHDATA2) {
  ...
  // 32 bit
    ...
    if (opcode !== require$$0$1.OP_PUSHDATA4) throw new Error('Unexpected opcode')
...
var map = {};
for (var op in require$$0$1) {
  var code = require$$0$1[op];
  map[code] = op;
}
...
function compile(chunks) {
  ...
  typeforce_1(types.Array, chunks);
...
function decompile(buffer) {
  ...
  typeforce_1(types.Buffer, buffer);
  const chunks = [];
  let i = 0;
...
function p2data(a, opts) {
  ...
  const network = a.network || networks.liquid;
...
function p2pkh(a, opts) {
  ...
  typeforce_1(
    {
      ...
      signature: typeforce_1.maybe(script.isCanonicalScriptSignature),
      input: typeforce_1.maybe(typeforce_1.Buffer),
      blindkey: typeforce_1.maybe(js.isPoint),
      confidentialAddress: typeforce_1.maybe(typeforce_1.String),
    },
    a,
  );
  ...
  lazy.prop(o, 'hash', () => {
    ...
    if (a.pubkey || o.pubkey) return crypto$1.hash160(a.pubkey || o.pubkey);
  ...
  if (opts.validate) {
    ...
    if (a.pubkey) {
      const pkh = crypto$1.hash160(a.pubkey);
    ...
    if (a.input) {
      ...
      if (a.pubkey && !a.pubkey.equals(chunks[1]))
        throw new TypeError('Pubkey mismatch');
      const pkh = crypto$1.hash160(chunks[1]);
      if (hash.length > 0 && !hash.equals(pkh))
        throw new TypeError('Hash mismatch');
    }
    if (a.confidentialAddress) {
    ...
    if (a.blindkey) {
      if (!js.isPoint(a.blindkey)) throw new TypeError('Blindkey is invalid');
...
function p2sh(a, opts) {
  ...
  if (!network) {
    network = (a.redeem && a.redeem.network) || networks.liquid;
  }
  const o = { network };
  const _address = lazy.value(() => {
    const payload = bs58check.decode(a.address);
  ...
  lazy.prop(o, 'hash', () => {
    ...
    if (a.address) return _address().hash;
    if (o.redeem && o.redeem.output) return crypto$1.hash160(o.redeem.output);
    if (a.confidentialAddress) {
      const address = _confidentialAddress().unconfidentialAddress;
      return bs58check.decode(address).slice(1);
    }
  ...
  if (opts.validate) {
    ...
    const checkRedeem = redeem => {
      ...
      if (redeem.output) {
        ...
        const hash2 = crypto$1.hash160(redeem.output);
        if (hash.length > 0 && !hash.equals(hash2))
          throw new TypeError('Hash mismatch');
        else hash = hash2;
      }
...
function p2wpkh(a, opts) {
  ...
  const _confidentialAddress = lazy.value(() => {
    const result = address.fromBlech32(a.confidentialAddress);
    return {
  ...
  lazy.prop(o, 'hash', () => {
    ...
    if (a.confidentialAddress) {
      ...
      return address.fromBech32(addr).data;
    }
  });
  lazy.prop(o, 'output', () => {
    if (!o.hash) return;
  ...
  lazy.prop(o, 'confidentialAddress', () => {
    if (!o.address) return;
    if (!o.blindkey) return;
    const res = address.fromBech32(o.address);
    const data = Buffer.concat([
      Buffer.from([res.version, res.data.length]),
      res.data,
    ]);
    return address.toBlech32(data, o.blindkey, o.network.blech32);
  });
  // extended validation
  if (opts.validate) {
    let hash = Buffer.from([]);
    ...
    if (a.witness) {
      ...
      if (!js.isPoint(a.witness[1]))
        throw new TypeError('Witness has invalid pubkey');
      if (a.signature && !a.signature.equals(a.witness[0]))
        throw new TypeError('Signature mismatch');
      if (a.pubkey && !a.pubkey.equals(a.witness[1]))
        throw new TypeError('Pubkey mismatch');
      const pkh = crypto$1.hash160(a.witness[1]);
      if (hash.length > 0 && !hash.equals(pkh))
        throw new TypeError('Hash mismatch');
    }
    if (a.confidentialAddress) {
...
function p2wsh(a, opts) {
  ...
  lazy.prop(o, 'hash', () => {
    ...
    if (a.address) return _address().data;
    if (o.redeem && o.redeem.output) return crypto$1.sha256(o.redeem.output);
    if (a.confidentialAddress) {
      const addr = _confidentialAddress().unconfidentialAddress;
      return address.fromBech32(addr).data;
    }
  ...
  if (opts.validate) {
    ...
    if (a.redeem) {
      ...
      if (a.redeem.output) {
        ...
        // match hash against other sources
        const hash2 = crypto$1.sha256(a.redeem.output);
        if (hash.length > 0 && !hash.equals(hash2))
          throw new TypeError('Hash mismatch');
        else hash = hash2;
      }
...
function toBase58Check(hash, version) {
  typeforce_1(types.tuple(types.Hash160bit, types.UInt8), arguments);
  const payload = Buffer.allocUnsafe(21);
...
class BufferWriter {
  constructor(buffer, offset = 0) {
    this.buffer = buffer;
    this.offset = offset;
    typeforce_1(types.tuple(types.Buffer, types.UInt32), [buffer, offset]);
  }
  writeUInt8(i) {
...
function nonceHash(pubkey, privkey) {
  return crypto$1.sha256(secp256k1.ecdh.ecdh(pubkey, privkey));
}
...
function fromPrivateKey(buffer, options) {
  typeforce_1(types.Buffer256bit, buffer);
...
function makeRandom(options) {
  ...
  do {
    d = rng(32);
    typeforce_1(types.Buffer256bit, d);
...
class Transaction {
  ...
  static isCoinbaseHash(buffer) {
    typeforce_1(types.Hash256bit, buffer);
    for (let i = 0; i < 32; ++i) {
  ...
  validateIssuance(assetBlindingNonce, assetEntropy, assetAmount, tokenAmount) {
    typeforce_1(types.Hash256bit, assetBlindingNonce);
    typeforce_1(types.Hash256bit, assetEntropy);
    typeforce_1(
      types.oneOf(
  ...
  hashForSignature(inIndex, prevOutScript, hashType) {
    ...
    return crypto$1.hash256(buffer);
  }
  hashForWitnessV0(inIndex, prevOutScript, value, hashType) {
    ...
    function writeInputs(ins) {
      ...
      const tBufferWriter = new bufferutils.BufferWriter(tBuffer, 0);
      ins.forEach(txIn => {
        tBufferWriter.writeSlice(txIn.hash);
        tBufferWriter.writeUInt32(txIn.index);
      });
      return crypto$1.hash256(tBuffer);
    }
  ...
  setInputScript(index, scriptSig) {
    typeforce_1(types.tuple(types.Number, types.Buffer), arguments);
  ...
  setInputIssuanceRangeProof(index, issuanceRangeProof) {
    typeforce_1(types.tuple(types.Buffer), arguments);
      ...
      (hasWitnesses
        ? this.ins.reduce((sum, input) => {
            return (
              ...
              varuintBitcoin.encodingLength(input.witness.length) +
              input.witness.reduce((scriptSum, scriptWit) => {
                return scriptSum + varSliceSize(scriptWit);
              }, 0) +
              varuintBitcoin.encodingLength(input.peginWitness.length) +
...
class Block {
  ...
  static fromBuffer(buffer) {
    ...
    const readVarInt = () => {
      const vi = varuintBitcoin.decode(buffer, offset);
  ...
  static calculateMerkleRoot(transactions, forWitness) {
    typeforce_1([{ getHash: types.Function }], transactions);
    ...
    const rootHash = fastRoot(hashes, crypto$1.hash256);
    return forWitness
      ? crypto$1.hash256(
  ...
  getHash() {
    return crypto$1.hash256(this.toBuffer(true));
  }
  getId() {
...
function decode(keyVal) {
  ...
  if (_offset < keyVal.value.length) {
    const rangeProofLen = varint.decode(keyVal.value, _offset);
    _next += varint.encodingLength(rangeProofLen);
    rangeProof = keyVal.value.slice(_offset, _next);
    _offset = _next;
    const surjectionProofLen = varint.decode(keyVal.value, _offset);
    _next += varint.encodingLength(surjectionProofLen);
...
function encode(data) {
  ...
  resultLen += nonceLen;
  varint.encode(script.length, result, resultLen);
  resultLen += varintLen;
  script.copy(result, resultLen);
  if (rangeProof && surjectionProof) {
    varint.encode(rangeProof.length, result, resultLen);
    resultLen += varint.encodingLength(rangeProofLen);
    rangeProof.copy(result, resultLen);
    varint.encode(surjectionProof.length, result, resultLen);
    resultLen += varint.encodingLength(surjectionProofLen);
...
class Psbt {
  ...
  updateGlobal(updateData) {
    utils.updateGlobal(updateData, this.globalMap);
    return this;
  }
  updateInput(inputIndex, updateData) {
    const input = utils.checkForInput(this.inputs, inputIndex);
    utils.updateInput(updateData, input);
    return this;
  }
  updateOutput(outputIndex, updateData) {
    const output = utils.checkForOutput(this.outputs, outputIndex);
    utils.updateOutput(updateData, output);
    return this;
  }
  addUnknownKeyValToGlobal(keyVal) {
    utils.checkHasKey(
      ...
      utils.getEnumLength(typeFields.GlobalTypes),
    );
  ...
  addUnknownKeyValToInput(inputIndex, keyVal) {
    ...
    utils.checkHasKey(
      ...
      utils.getEnumLength(typeFields.InputTypes),
    );
    if (!input.unknownKeyVals) input.unknownKeyVals = [];
    input.unknownKeyVals.push(keyVal);
    return this;
  ...
  addUnknownKeyValToOutput(outputIndex, keyVal) {
    ...
    utils.checkHasKey(
      ...
      utils.getEnumLength(typeFields.OutputTypes),
    );
    if (!output.unknownKeyVals) output.unknownKeyVals = [];
    output.unknownKeyVals.push(keyVal);
    return this;
  ...
  addInput(inputData) {
    ...
    utils.addInputAttributes(this.inputs, inputData);
    return this;
  }
  addOutput(outputData) {
    ...
    utils.addOutputAttributes(this.outputs, outputData);
    return this;
  }
  clearFinalizedInput(inputIndex) {
    const input = utils.checkForInput(this.inputs, inputIndex);
    utils.inputCheckUncleanFinalized(inputIndex, input);
  ...
  updateInput(inputIndex, updateData) {
    if (updateData.witnessUtxo) {
      ...
      const asset = Buffer.isBuffer(witnessUtxo.asset)
        ...
        : Buffer.concat([
            Buffer.alloc(1, 1),
            bufferutils.reverseBuffer(Buffer.from(witnessUtxo.asset, 'hex')),
          ]);
...
class PsbtTransaction {
  ...
  addOutput(output) {
    ...
    const asset = Buffer.isBuffer(output.asset)
      ...
      : Buffer.concat([
          Buffer.alloc(1, 1),
          bufferutils.reverseBuffer(Buffer.from(output.asset, 'hex')),
        ]);
...
function checkScriptForPubkey(pubkey, script$1, action) {
  const pubkeyHash = crypto$1.hash160(pubkey);
...
var __pika_web_default_export_for_treeshaking__ = /*@__PURE__*/getDefaultExportFromCjs(src$1);

var Block = src$1.Block;
var ECPair = src$1.ECPair;
var Psbt = src$1.Psbt;
var Transaction = src$1.Transaction;
var address$1 = src$1.address;
var bip32$1 = src$1.bip32;
var confidential$1 = src$1.confidential;
var crypto$2 = src$1.crypto;
export default __pika_web_default_export_for_treeshaking__;
var networks$1 = src$1.networks;
var opcodes = src$1.opcodes;
var payments$1 = src$1.payments;
var script$1 = src$1.script;
export { Block, ECPair, Psbt, Transaction, address$1 as address, bip32$1 as bip32, confidential$1 as confidential, crypto$2 as crypto, networks$1 as networks, opcodes, payments$1 as payments, script$1 as script };
